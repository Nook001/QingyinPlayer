...
- 已启用外键约束，并将数据库 schema 版本设为 `2`；打开旧库时会重建搜索索引。
...
- 曲库列表使用可排序曲目表格；歌曲名、专辑和时长表头支持升序/降序切换。
...
| Storage | SQLite schema、事务 upsert、列表、删除和分字段拼音搜索 | 尚无聚合查询 |
| Chinese | 全拼与首字母搜索键 | ICU4X 排序、`ARTISTSORT` 和多音字覆盖未实现 |
| UI Bridge | 曲库模型、后台扫描、搜索、排序、播放控制、EOS 与错误处理 | 尚无细粒度扫描进度和聚合模型 |
| QML | 曲库内搜索、排序、封面、连续播放、完整播放栏、导航和双主题 | 歌手和专辑尚未贯通，主题未持久化 |
