...
顺序、单曲循环和随机是播放模式，作用在当前点播队列上。正在播放列表按这个顺序展示，重启后不保留。歌单只记录曲目成员，查看时默认按标题升序，播放时把当时的视图顺序复制进队列。

歌词页是盖在曲库上的一层，并接住自己的指针事件。窗口背景由颜色主题驱动，候选项是青瓷、霁蓝、松烟和暮山。悬停和按下使用前景色的半透明叠层，而不是固定色块。
