...
顺序、单曲循环和随机是播放模式，作用在当前点播队列上。正在播放列表按这个顺序展示，退出时会记下这份顺序。歌单只记录曲目成员，查看时默认按标题升序，播放时把当时的视图顺序复制进队列。

歌词页是盖在曲库上的一层，并接住自己的指针事件。退出或关闭主窗口前记下当前播放顺序、当前曲目、进度，以及这次播放来自哪一个歌单。下次启动按这个顺序恢复播放列表，并暂停在上次的位置；歌单仍会打开，但不会把临时队列改回标题顺序。关闭主窗口只是隐藏，进程留在系统托盘里，从托盘菜单退出才会结束。窗口背景由颜色主题驱动，候选项是青瓷、霁蓝、松烟和暮山。悬停和按下使用前景色的半透明叠层，而不是固定色块。
...
`tracks` 保存路径、展示名、时长、指纹、排序标签、封面摘要，以及格式、采样率、位深、声道和码率。歌手在 `track_artists`，搜索键在 `search_terms`。歌单在 `playlists` 与 `playlist_tracks`，成员没有播放顺序，曲目删除时一并移除。封面原图不入库，按摘要缓存在磁盘上。设置里的 `color_theme` 保存颜色主题；缺失该字段时，旧的 `dark_theme` 迁到松烟或青瓷。
