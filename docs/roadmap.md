...
播放使用 GStreamer。底部播放条负责进度、音量和上一首／下一首；正在播放列表显示实际播放顺序。歌词页读取同名 `.lrc` 或内嵌歌词；胶囊窗口与主窗口共用播放状态。曲目行标出 Hi-Res。歌单只保存成员。窗口无边框，颜色主题取代浅色／深色开关，QML 已嵌入可执行文件，并带有 `.desktop` 与安装脚本。检查在本地运行，仓库不再附带 GitHub Actions。
